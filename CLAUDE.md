...
- `src/bots/agent/**` — the concierge's two callable actions plus internal-only bots:
  ...
  - `agent-patient-chat.ts`, `agent-ensure-doctor.ts`, `agent-find-doctors.ts`: supporting tools (doctor-desk chat, synthetic-provider creation; `agent-find-doctors`'s search helpers are also called directly by `agent-booking-chat`'s tools). `agent-find-doctors.ts`/`agent-ensure-doctor.ts` also still back a separate, disconnected manual flow (`DoctorResultsPage`/`SlotPickerPage`) that predates the chat and isn't linked from it.
